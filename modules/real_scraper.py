...
System 5.0.2 - Scrapes REAL data from Google Maps for hedge fund intelligence

Multi-sort review fetching: The Google Places API returns at most 5 reviews
per request. By making two requests with different sort orders (most_relevant
and newest) and deduplicating, we can get up to 10 unique reviews per location.
...
# Sort orders supported by Google Places API (legacy)
# Each returns up to 5 reviews; combining both yields up to 10 unique reviews
REVIEW_SORT_ORDERS = ["most_relevant", "newest"]
...
class GoogleMapsRealScraper:
    ...
    Multi-sort review fetching (v5.0.2):
    The Google Places API returns at most 5 reviews per request.
    By default, get_place_details() now makes two API calls with different
    sort orders (most_relevant + newest) and deduplicates the results,
    yielding up to 10 unique reviews per location instead of 5.
    """
    ...
    def get_place_details(
        self,
        place_id: str,
        multi_sort_reviews: bool = True
    ) -> Optional[PlaceData]:
        """
        Get detailed information about a place.

        The Google Places API returns at most 5 reviews per request.
        When multi_sort_reviews=True (default), we make two requests with
        different sort orders (most_relevant + newest) and deduplicate,
        yielding up to 10 unique reviews per location.

        Args:
            place_id: Google Place ID
            multi_sort_reviews: If True, fetch reviews with multiple sort
                orders and merge/deduplicate for up to 10 reviews.
                If False, use single request (legacy behavior, max 5).

        Returns:
            PlaceData object or None
        """
        # Check cache first (cache key includes multi_sort flag)
        cache_key = f"{place_id}:ms" if multi_sort_reviews else place_id
        if self.cache:
            cached = self.cache.get(cache_key)
            if cached:
                logger.debug("cache_hit", place_id=place_id, multi_sort=multi_sort_reviews)
                return PlaceData(**cached)

        if multi_sort_reviews:
            place_data = self._fetch_with_multi_sort_reviews(place_id)
        else:
        ...
        if place_data:
            # Cache the result
            if self.cache:
                self.cache.set(cache_key, place_data.to_dict())
    ...
    def get_place_reviews(
        ...
        Args:
            place_id: Google Place ID
            multi_sort: If True, use multiple sort orders for up to 10 reviews.

        Returns:
            List of ReviewData objects
        """
        place_data = self.get_place_details(place_id, multi_sort_reviews=multi_sort)
    ...
    def _fetch_from_api(
        ...
        """Fetch place details from Google Maps API with a specific review sort order.

        Args:
            place_id: Google Place ID
            reviews_sort: Sort order for reviews. One of 'most_relevant' (default)
                or 'newest'.
    ...
    def _fetch_with_multi_sort_reviews(self, place_id: str) -> Optional[PlaceData]:
        """Fetch place details with multiple review sort orders and merge results.

        Makes one API call per sort order (most_relevant, newest) and deduplicates
        the reviews by (author_name, time) combination. This yields up to 10 unique
        reviews per location instead of the default 5.

        The place metadata (name, address, rating, etc.) is taken from the first
        successful API response. Only the reviews are merged across all responses.

        Args:
            place_id: Google Place ID

        Returns:
            PlaceData object with merged reviews, or None if all calls fail
        """
        base_place_data: Optional[PlaceData] = None
        all_reviews: List[Dict] = []
        seen_review_keys = set()

        for sort_order in REVIEW_SORT_ORDERS:
            try:
                place_data = self._fetch_from_api(place_id, reviews_sort=sort_order)
                if place_data is None:
                    continue

                # Use first successful response as the base for metadata
                if base_place_data is None:
                    base_place_data = place_data

                # Deduplicate reviews by (author_name, time) tuple
                for review in place_data.reviews:
                    # Build a dedup key from author_name + timestamp
                    ...
                    dedup_key = (author, review_time)

                    if dedup_key not in seen_review_keys:
                        seen_review_keys.add(dedup_key)
                        all_reviews.append(review)

            except Exception as e:
                logger.warning(
                    "multi_sort_fetch_error",
                    place_id=place_id,
        ...
        # Replace reviews with the merged, deduplicated set
        base_place_data.reviews = all_reviews

        logger.debug(
            "multi_sort_reviews_merged",
            place_id=place_id,
            sort_orders=len(REVIEW_SORT_ORDERS),
            unique_reviews=len(all_reviews)
        )

        return base_place_data

    @staticmethod
    def _deduplicate_reviews(reviews: List[Dict]) -> List[Dict]:
        """Deduplicate reviews by (author_name, time) combination.

        Args:
            reviews: List of review dicts

        Returns:
            Deduplicated list of review dicts
        """
        seen = set()
        unique = []
